...
def estimate_pose(bboxs, ids, img, mtx, dist, xyz_map, marker_size=0.173):
    ...
    if rvecs is not None and tvecs is not None:
        for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
            ...
            cv2.putText(img,
                        f"ID={ids[i][0]} X={x:.1f} Y={y:.1f} Z={z:.1f} Yaw={yaw:.1f}° Dist={distance_cm:.1f}cm",
                        (10, 30 + i*30),
...
# =========================
def main():
    cam_mat, dist_coef = load_calibration_data_opencv("zed2_intrinsic.yaml")
